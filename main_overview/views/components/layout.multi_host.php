...
function render_multi_host_root(array $data): CDiv
{
    ...
    $list = (new CDiv())->addClass('main-overview-multi-list');

    $detail_view = (new CDiv())
        ->addClass('main-overview-multi-detail-view')
    ...
    foreach ($hosts as $host) {
        ...
        $summary = (new CDiv())
            ->addClass('main-overview-multi-summary')
            ->setAttribute('data-main-overview-nav', $hostid)
            ->setAttribute('role', 'button')
            ->setAttribute('tabindex', '0')
            ->setAttribute('aria-label', _s('Открыть детали: %1$s', $label));

        $left = (new CDiv())->addClass('main-overview-multi-summary-main');

        $left->addItem(
            (new CSpan())
                ->addClass('main-overview-multi-name')
                ->addItem($label)
        );
        ...
        if ($summary_toolbar !== null) {
            $summary_toolbar->addClass('main-overview-multi-summary-toolbar');
        ...
        $summary->addItem(
            (new CSpan())
                ->addClass('main-overview-light')
                ->addClass('main-overview-light--' . $light)
        ...
        $panel = (new CDiv())
            ->addClass('main-overview-detail-panel')
