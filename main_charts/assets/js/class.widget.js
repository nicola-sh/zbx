...
class CWidgetMainCharts extends CWidget {

  static ACTION_HISTORY = 'widget.main_charts.history';
  ...
  _getWidgetRoot() {
    return this._body?.closest('.dashboard-widget-main_charts')
      || this._target
      || this._body;
  }
  ...
  _observeCanvas() {
    const wrap = this._body?.querySelector('.main-charts-canvas-wrap');
  ...
  _buildChartDatasets(payload, config, theme) {
    ...
    const fillUnderLine = chartTypeId === CWidgetMainCharts.CHART_TYPE_AREA
      || (chartTypeId === CWidgetMainCharts.CHART_TYPE_LINE && String(config.chart_fill) === '1');
  ...
  _resolveChartType(value) {
    ...
    switch (numeric) {
      case CWidgetMainCharts.CHART_TYPE_AREA:
        return 'line';
      case CWidgetMainCharts.CHART_TYPE_BAR:
        return 'bar';
      default:
        return 'line';
    }
  ...
  _showError(message) {
    const stage = this._body?.querySelector('.main-charts-stage');

    if (!stage) {
      return;
    }

    let banner = stage.querySelector('.main-charts-error');

    if (!banner) {
      banner = document.createElement('div');
      banner.className = 'main-charts-error';
  ...
  _destroyChart() {
    ...
    this._body?.querySelector('.main-charts-error')?.remove();
  }
}
