...
class AOverviewSparkline {

  static ACTION = 'widget.aoverview.sparkline';
  ...
  async _fetchSparklineData(spec, period, signal) {
    ...
    curl.setArgument('action', AOverviewSparkline.ACTION);
  ...
  _getOverlayRoot() {
    return (typeof this.options.getOverlayRoot === 'function'
      ...
      : null) || this._getBody()?.querySelector('[data-a-overview-role="sparkline"]');
  }

  _getOverviewRoot() {
    return (typeof this.options.getOverviewRoot === 'function'
      ? this.options.getOverviewRoot()
      : null) || this._getBody()?.querySelector('[data-a-overview-role="overview"]');
  }
