...
class CWidgetAOverview extends CWidget {
  ...
  static TRAFFIC_LIGHT_CLASSES = ['a-overview-light--green', 'a-overview-light--yellow', 'a-overview-light--red'];
  ...
  onInitialize() {
    ...
    this._sparkline = new AOverviewSparkline({
      getBody: () => this._body,
  ...
  _getWidgetRoot() {
    return this._body?.closest('.dashboard-widget-aoverview')
      || this._target
      || this._body;
  }

  _getFirstOverviewRoot() {
    return this._body?.querySelector('[data-a-overview-role="overview"]') || null;
  }

  _getSparklineRoot() {
    return this._body?.querySelector('[data-a-overview-role="sparkline"]') || null;
  }
  ...
  _attachOverview() {
    const next = new Set(this._body?.querySelectorAll('[data-a-overview-role="overview"]') || []);
  ...
  _handleMultiNavClick(event) {
    const summary = event.target.closest('[data-a-overview-nav]');
    ...
    const hostid = summary.getAttribute('data-a-overview-nav');
  ...
  _openMultiDetail(hostid) {
    const root = this._getWidgetRoot();
    const multi = this._body?.querySelector('[data-a-overview-multi="1"]');

    if (!root || !multi || !hostid) {
      return;
    }

    root.classList.add('a-overview--multi-detail');
    multi.querySelector('.a-overview-multi-list-view')?.setAttribute('hidden', 'hidden');
    multi.querySelector('.a-overview-multi-detail-view')?.removeAttribute('hidden');
  ...
  _showMultiListView() {
    ...
    root?.classList.remove('a-overview--multi-detail');
    multi.querySelector('.a-overview-multi-detail-view')?.setAttribute('hidden', 'hidden');
    multi.querySelector('.a-overview-multi-list-view')?.removeAttribute('hidden');
  ...
  _applyTrafficLight(element, light) {
    ...
    element.classList.remove(...CWidgetAOverview.TRAFFIC_LIGHT_CLASSES);
    element.classList.add(`a-overview-light--${safe}`);
  ...
  _patchMultiHostValues(hostsMap) {
    ...
    for (const [hostid, payload] of Object.entries(hostsMap)) {
      const summary = root.querySelector(`[data-a-overview-nav="${this._escapeSelector(hostid)}"]`);
      const lightEl = summary?.querySelector('.a-overview-light');

      if (lightEl && payload?.light) {
        this._applyTrafficLight(lightEl, payload.light);
      }

      const summaryToolbar = summary?.querySelector('.a-overview-multi-summary-toolbar');
      ...
      const overview = panel?.querySelector('[data-a-overview-role="overview"]');
  ...
  _applyStateClasses(element, classes) {
    ...
    const toRemove = [...new Set([
      ...CWidgetAOverview.DYNAMIC_BADGE_CLASSES,
      ...previous,
    ])];
  ...
  _onOverviewClick(event) {
    ...
    const root = bar.closest('[data-a-overview-role="overview"]');
